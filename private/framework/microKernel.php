...
\*Author: Jossue O. Rodriguez C.   $LastChangedRevision: 232 $
...
*/
	...
	{
		if ($handle = opendir(PATH_PRIVATE . $in)) {
			
		    while (false !== ($entry = readdir($handle))) {
		    	
		        if (!is_dir(PATH_PRIVATE . $in . $entry)) {
	...
	function loadClassIn($dir, $deep=false){
		if(is_file(PATH_PRIVATE . $dir)){
			
			loadClass($dir);
			
		}else if ($handle = opendir(PATH_PRIVATE . $dir)) {
			
		    while (false !== ($entry = readdir($handle))) {
		    	
		        if (!is_dir(PATH_PRIVATE . $dir . $entry)) {
					loadClass($dir . $entry);					
	...
	function loadClass($filename, $searchClassName=false){
		...
		if(class_exists($className)){
			...
			if(is_file(PATH_PRIVATE . $filename) ){
				//echo var_dump("cargando: " . PATH_PRIVATE . $filename);
				include PATH_PRIVATE . $filename;
	...
	loadClassIn(PATH_FRAMEWORK .'models/');
	loadClassIn(PATH_FRAMEWORK .'components/');
	
	//echo "microkernel loaded <br />";
?>
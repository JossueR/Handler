...
class RolHandler extends Handler {
	...
	function __construct(){
		...
		$this->squema_workspace= "views/rol/listWorkspace.php";
	...
	public function storeAction(){
		...
		if(!$this->haveErrors() && $rol->save($proto)){
			$this->asyncLoad('Rol', Environment::$APP_CONTENT_BODY, array(
				'do'=>'listWorkspace'
			));
		}else{
			$cols = $this->labelsDefinition();
	...
	function dashAsocAction($title = "rols"){
		...
		$this->setVar("link_assoc", Handler::asyncLoad($this->getHandlerName(), "dash_assoc_rol", array("do" => "list", "reff_id" => $reff_id, "type"=> self::LIST_BY_USER_NOSELECTED),true));
		$this->setVar("link_view",  Handler::asyncLoad($this->getHandlerName(), "dash_assoc_rol", array("do" => "list", "reff_id" => $reff_id, "type"=> self::LIST_BY_USER_SELECTED),true));
	...
	function assocToUserAction(){
		...
		if($dao->addToUser($user_id, $rol_id)){
			Handler::asyncLoad($this->getHandlerName(), "dash_assoc_rol", array(
				"do" => "list",
				"reff_id" => $user_id,
